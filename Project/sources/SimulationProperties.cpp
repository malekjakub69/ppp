...
void SimulationProperties::parseCommandLine(int argc, char *argv[])
{
  ...
  options.add_options()("m,mode", "Simulation mode\n"
                                  "  0 - run sequential version\n"
                                  "  1 - run parallel version point-to-point\n"
                                  "  2 - run parallel version RMA",
                        cxxopts::value<int>(), "<int>")("n,iterations", "Number of iterations", cxxopts::value<std::size_t>(), "<uint>")("i,input", "Input material file name", cxxopts::value<std::string>(), "file")("t,threads", "Number of OpenMP threads", cxxopts::value<int>()->default_value("1"), "<uint>")("o,output", "Output HDF5 file name", cxxopts::value<std::string>()->default_value(""), "<string>")("w,write-intensity", "Disk write intensity", cxxopts::value<std::size_t>()->default_value("50"), "<uint>")("a,airflow-rate", "Air flow rate (values in <0.0001, 0.5> make sense)",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   cxxopts::value<float>()->default_value("0.001f"), "<float>")("d,debug", "Enable debugging (copare results of SEQ and PAR versions)")("v,verify", "Verification mode (compare results of SEQ and PAR versions)")("b,batch", "Batch mode")("B,batch-with-header", "Batch mode with header")("p,parallel-io", "Parallel I/O mode")("r,render", "Render results into *.png image", cxxopts::value<std::string>(), "<string>")("g,decomp-2d", "Use 2D decomposition instead of 1D")("h,help", "Print usage");
  ...
  catch (const std::exception &e)
  {
    ...
    if (rank == 0)
    {
      std::cerr << "Arguments parse error: " << e.what() << std::endl
                << std::endl;
