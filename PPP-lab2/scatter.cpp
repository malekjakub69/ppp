...
int main(int argc, char **argv)
{
  ...
  case 1: // Example 1 - Broadcast your name.
  {
    ...
    int localSizeA = size / mpiGetCommSize(MPI_COMM_WORLD);
    int localSizeB = size / mpiGetCommSize(MPI_COMM_WORLD);

    // 3. Allocate local arrays at every rank using std::vector.

    std::vector<int> localA(localSizeA);
    std::vector<int> localB(localSizeB);

    // 4. Scatter vector a and b into local vectors.

    MPI_Scatter(a.data(), localSizeA, MPI_INT, localA.data(), localSizeA, MPI_INT, MPI_ROOT_RANK, MPI_COMM_WORLD);
    MPI_Scatter(b.data(), localSizeB, MPI_INT, localB.data(), localSizeB, MPI_INT, MPI_ROOT_RANK, MPI_COMM_WORLD);
  ...
  case 2: // Example 2 - Broadcast your name using 2 Sends and Recvs.
  {
    ...
    int reminder = size % mpiGetCommSize(MPI_COMM_WORLD);
    ...
    {
      sendCounts[i] = (i < reminder) ? localSizeA + 1 : localSizeA;
    }
    ...
    std::vector<int> localA(sendCounts[mpiGetCommRank(MPI_COMM_WORLD)]);
    std::vector<int> localB(sendCounts[mpiGetCommRank(MPI_COMM_WORLD)]);

    // 8. Scatter vector a and b into local vectors using Scatterv

    MPI_Scatterv(a.data(), sendCounts.data(), displacements.data(), MPI_INT,
                 localA.data(), sendCounts[mpiGetCommRank(MPI_COMM_WORLD)],
                 MPI_INT, MPI_ROOT_RANK, MPI_COMM_WORLD);

    MPI_Scatterv(b.data(), sendCounts.data(), displacements.data(), MPI_INT,
                 localB.data(), sendCounts[mpiGetCommRank(MPI_COMM_WORLD)],
                 MPI_INT, MPI_ROOT_RANK, MPI_COMM_WORLD);

    // 9. Calculate local dot product

    int localResult = 0;
    for (int i = 0; i < sendCounts[mpiGetCommRank(MPI_COMM_WORLD)]; i++)
    {
      localResult += localA[i] * localB[i];
    }
