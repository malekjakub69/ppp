...
/// student's login
const std::string_view studentLogin = "xmalek17";
...
int main(int argc, char **argv)
{
  ...
  case 1:
  {
    ...
    MPI_Bcast(&myName[0], myNameLength, MPI_CHAR, MPI_ROOT_RANK, MPI_COMM_WORLD);

    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    // Print your name
    mpiPrintf(MPI_ALL_RANKS, "Rank %d: %s (%d characters).\n",
              mpiGetCommRank(MPI_COMM_WORLD), myName.c_str(), myNameLength);
  ...
  case 2:
  {
    ...
    if (mpiGetCommRank(MPI_COMM_WORLD) == MPI_ROOT_RANK)
    {
      ...
      {
        MPI_Send(&myNameLength, 1, MPI_INT, i, tagLength, MPI_COMM_WORLD);
        MPI_Send(myName.c_str(), myNameLength, MPI_CHAR, i, tagData, MPI_COMM_WORLD);
      }
    }
    ...
    if (mpiGetCommRank(MPI_COMM_WORLD) != MPI_ROOT_RANK)
    {
      MPI_Recv(&myNameLength, 1, MPI_INT, MPI_ROOT_RANK, tagLength, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      myName.resize(myNameLength);
      MPI_Recv(&myName[0], myNameLength, MPI_CHAR, MPI_ROOT_RANK, tagData, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  case 3: // Example 3 - Broadcast your name using 1 ISend and Get_count + Recv
  {
    ...
    if (mpiGetCommRank(MPI_COMM_WORLD) == MPI_ROOT_RANK)
    {
      MPI_Request req[mpiGetCommSize(MPI_COMM_WORLD)];
      req[MPI_ROOT_RANK] = MPI_REQUEST_NULL;
      for (int i = 1; i < mpiGetCommSize(MPI_COMM_WORLD); i++)
      {
        MPI_Isend(myName.c_str(), myNameLength, MPI_CHAR, i, tag, MPI_COMM_WORLD, &req[i]);
      }
      MPI_Waitall(mpiGetCommSize(MPI_COMM_WORLD), req, MPI_STATUSES_IGNORE);
    }

    // 3. Other processes first probe the incoming message (MPI_Probe).
    //    Then they learn the size of the incoming message by calling MPI_Get_count.
    //    Finally, they reserve message buffer and receive the message.

    if (mpiGetCommRank(MPI_COMM_WORLD) != MPI_ROOT_RANK)
    {
      MPI_Status status;
      MPI_Probe(MPI_ROOT_RANK, tag, MPI_COMM_WORLD, &status);
      MPI_Get_count(&status, MPI_CHAR, &myNameLength);
      myName.resize(myNameLength);
      MPI_Recv(&myName[0], myNameLength, MPI_CHAR, MPI_ROOT_RANK, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
