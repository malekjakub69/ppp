...
int main(int argc, char **argv)
{
  ...
  case 1:
  {
    ...
    MPI_File_open(MPI_COMM_WORLD, "File1.txt", MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &file);
    ...
    if (mpiGetCommRank(MPI_COMM_WORLD) == MPI_ROOT_RANK)
    {
      ...
      MPI_File_write(file, msg.data(), msg.size(), MPI_CHAR, MPI_STATUS_IGNORE);
    }
  ...
  case 2:
  {
    ...
    MPI_File_open(MPI_COMM_WORLD, "File2.txt", MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &file);

    // Declare a string and write "I am %d from %d\n"
    constexpr std::size_t maxStrSize{128};

    std::array<char, maxStrSize> str{};
    int size = std::snprintf(str.data(), maxStrSize, "I am %d from %d\n", mpiGetCommRank(MPI_COMM_WORLD),
    ...
    MPI_File_write_all(file, str.data(), size, MPI_CHAR, MPI_STATUS_IGNORE);
    // 5. Close the file.
  ...
  case 3:
  {
    ...
    MPI_Scatter(gMatrix.data(), lRows, ROW_TYPE, lMatrix.data(), lRows, ROW_TYPE, MPI_ROOT_RANK,
                MPI_COMM_WORLD);
    ...
    MPI_File_set_view(file, lRows * nCols * mpiGetCommRank(MPI_COMM_WORLD) * sizeof(int), MPI_INT, ROW_TYPE, "native", MPI_INFO_NULL);

    // 6. Use a collective write.

    MPI_File_write_all(file, lMatrix.data(), lRows, ROW_TYPE, MPI_STATUS_IGNORE);
  ...
  case 4:
  {
    ...
    MPI_File_open(MPI_COMM_WORLD, outFilename.data(), MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &outFile);
    ...
    if (mpiGetCommRank(MPI_COMM_WORLD) == MPI_ROOT_RANK)
    {
      ...
      MPI_File_read(inFile, buffer.data(), maxBufferSize, MPI_BYTE, MPI_STATUS_IGNORE);

      fileHeader = Header(buffer.data());

      // 4. Write the header into the output file, use fileHeader.size and fileHeader.toString().c_str().

      MPI_File_write(outFile, fileHeader.toString().c_str(), fileHeader.size, MPI_BYTE, MPI_STATUS_IGNORE);
    }

    // 5. Broadcast the header structure. Header is POD, can be sent as a contiguous block.

    MPI_Bcast(&fileHeader, sizeof(fileHeader), MPI_BYTE, MPI_ROOT_RANK, MPI_COMM_WORLD);

    // Get number of local rows and cols.
    lRows = fileHeader.nRows / static_cast<int>(std::sqrt(mpiGetCommSize(MPI_COMM_WORLD)));
    lCols = fileHeader.nCols / static_cast<int>(std::sqrt(mpiGetCommSize(MPI_COMM_WORLD)));

    // 6. Create a subarray to read/write a tile from/to the input/output file.

    std::vector<unsigned char> image(lRows * lCols);

    int localNRows, localNCols, nProcs;
    nProcs = mpiGetCommSize(MPI_COMM_WORLD);
    localNRows = localNCols = std::sqrt(nProcs);
    int myId = mpiGetCommRank(MPI_COMM_WORLD);

    int sizes[2] = {fileHeader.nRows, fileHeader.nCols};
    int subsizes[2] = {lRows, lCols};
    int starts[2] = {(myId / localNRows) * lRows,
                     (myId % localNCols) * lCols};

    // mpiPrintf(-1, "Rank %d: sizes = {%d, %d}, subsizes = {%d, %d}, starts = {%d, %d}\n", myId, sizes[0], sizes[1], subsizes[0], subsizes[1], starts[0], starts[1]);
    ...
    MPI_File_set_view(inFile, fileHeader.size, MPI_BYTE, BLOCK_TYPE, "native", MPI_INFO_NULL);

    mpiPrintf(MPI_ROOT_RANK, " Reading input file...\n");
    // 8. Read input image.

    MPI_File_read(inFile, image.data(), lCols * lRows, MPI_BYTE, MPI_STATUS_IGNORE);
    ...
    for (int row = 0; row < lRows; row++)
    {
      ...
      {
        double corrected = 255.0 * std::pow(image[row * lCols + col] / 255.0, 1.0 / 2.2);

        image[row * lCols + col] = static_cast<unsigned char>(std::min(corrected, 255.0));
      }
    }
    ...
    MPI_File_set_view(outFile, fileHeader.size, MPI_BYTE, BLOCK_TYPE, "native", MPI_INFO_NULL);
    MPI_File_write(outFile, image.data(), lCols * lRows, MPI_BYTE, MPI_STATUS_IGNORE);
